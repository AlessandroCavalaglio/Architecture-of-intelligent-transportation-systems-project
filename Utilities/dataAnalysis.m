...
%% Plot vehicle motion
% ---------------------------------
figure('Name','Vehicle motion','NumberTitle','off'), clf
% --- u --- %
ax(1) = subplot(221);
...
% set( gca, 'fontsize', 16 );
% hold on; grid on;
% plot( time_sim, u_est * 3.6, 'LineWidth', 1.29 );
% plot( time_sim, u_est_selector * 10, 'LineWidth', 1.29 );
% xlabel( 'time [s]' );
% ylabel( 'longitudinal velocity [m/s]' );
% legend( 'Longitudinal velocity', 'Estimated longitudinal velocity', 'Selector $10 = u_4$, $20 = u_{front}$, $30 = u_{acc}$' , ...
%         'location', 'best');
% title( 'Longitudinal velocity estimation', 'FontSize', 18 );
...
fprintf('Performance: %.2f \n', model_sim.performance.Data(end));
